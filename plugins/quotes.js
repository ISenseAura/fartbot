...
let commands = {
	...
	addquote: function (target, room, user) {
		...
		let to = Tools.toId("Lego");
		let message = "add quote for room: " + room.id + ": " + target;
		let db = Storage.getDatabase('global');
		if (!(to in db.mail)) {
			db.mail[to] = [];
		}
		db.mail[to].push({time: Date.now(), from: user.name, text: message});
		Storage.exportDatabase('global');
		this.say("Quote submitted for review!");
	},
	removequote: function (target, room, user) {
		...
		let to = Tools.toId("Lego");
		let message = "remove quote from room: " + this.room.name + ": " + target;
		let db = Storage.getDatabase('global');
		if (!(to in db.mail)) {
			db.mail[to] = [];
		}
